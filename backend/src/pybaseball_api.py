...
from pybaseball import get_splits, playerid_reverse_lookup, batting_stats
...
def get_player(mlbamID: str):
    ...
    response = json.jsonify({"result": batting_stats(start_season=1871, end_season=datetime.now().year, players=player_ids["key_fangraphs"][0]).sort_values(["Season"]).to_json(orient="records")})
